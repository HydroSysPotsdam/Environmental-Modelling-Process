...
def plot_convergence(S, NN, S_lb=np.array([]), S_ub=np.array([]),
    ...
    colorscale = 'jet'
    col =  eval('plt.cm.'+colorscale)(np.linspace(0,1,S.shape[1])) # np.linspace(0,1,5)
    ...
    if  len(S_lb) != 0:
        V1 = np.min(S_lb.flatten() - 0.05)
    else:
        V1 = min(-0.1, np.min(S.flatten()))
    if  len(S_ub) != 0:
        V2 = np.max(S_ub.flatten() + 0.05) # max(1.1, np.max(S_ub.flatten()))
    else:
